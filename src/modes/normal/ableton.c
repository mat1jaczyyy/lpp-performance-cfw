	...
	if (p == 0) { // Enter Setup mode
		...
		switch (ableton_layout) {
			...
			case ableton_layout_user: // User mode
				if (x == 9) {
					send_midi(USBMIDI, 0xB0 | settings.mode[mode_ableton].channel, p, v);
				} else {
					send_midi(USBMIDI, (v? 0x90 : 0x80) | settings.mode[mode_ableton].channel, xy_dr[p], v);
				}
				break;
		}
	}
	...
	if (port == USBMIDI) {
		...
		switch (t) {
			...
			case 0x9:
				...
				if (ableton_layout == ableton_layout_user && (ch == settings.mode[mode_ableton].channel || ch == 0x1 || ch == 0x2)) {
			...
			case 0xB:
				if (ableton_layout == ableton_layout_user) {
					if (ch == settings.mode[mode_ableton].channel) ableton_led(ch, p, v, 1);
				} else {
...
void ableton_aftertouch_event(u8 v) {
	aftertouch_send(USBMIDI, 0xD0 | (ableton_layout == ableton_layout_user? 0x0 : settings.mode[mode_ableton].channel), v);
}

void ableton_poly_event(u8 p, u8 v) {
	send_midi(USBMIDI, 0xA0 | (ableton_layout == ableton_layout_user? 0x0 : settings.mode[mode_ableton].channel), p, v);
}