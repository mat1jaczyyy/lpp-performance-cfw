...
void drum_init() {
    ...
    for (u8 i = 0; i < 128; i++) { // Turn off all notes
        send_midi(USBSTANDALONE, 0x80 | settings.mode[mode_drum].channel, i, 0);
    }
...
void drum_surface_event(u8 p, u8 v, u8 x, u8 y) {
    if (p == 0) { // Enter Setup mode
        ...
        send_midi(USBSTANDALONE, 0xB0 | settings.mode[mode_drum].channel, p, v);
        ...
        send_midi(USBSTANDALONE, (v? 0x90 : 0x80) | settings.mode[mode_drum].channel, drum_press(x, y, v, -1), v);
    }
}

void drum_midi_event(u8 port, u8 t, u8 ch, u8 p, u8 v) {
    if (port == USBSTANDALONE && ch == settings.mode[mode_drum].channel) {
        u8 x = p / 10;
        u8 y = p % 10;
...
void drum_aftertouch_event(u8 v) {
    aftertouch_send(USBSTANDALONE, 0xD0 | settings.mode[mode_drum].channel, v);
}

void drum_poly_event(u8 p, u8 v) {
    send_midi(USBSTANDALONE, 0xA0 | settings.mode[mode_drum].channel, drum_press(p / 10, p % 10, v, -1), v);
}
